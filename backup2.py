...
def validate_extension(filename: str) -> str:
    ...
    if ext == ".pdf":
        raise HTTPException(
            status_code=400,
            detail="PDF files are not supported. Please upload a PNG, JPG, JPEG, or WEBP file."
        )
...
def validate_position(position: str) -> dict:
    if position not in ZONES:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid position. Allowed positions: {', '.join(ZONES.keys())}"
        )
    return ZONES[position]


def get_garment_template_for_position(position: str) -> Path:
    zone = validate_position(position)

    if zone["view"] == "front":
        garment_path = ASSETS_DIR / "garment_front.jpg"
    else:
        garment_path = ASSETS_DIR / "garment_back.jpg"

    if not garment_path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"{garment_path.name} not found in assets/"
        )
...
def render_preview(
    ...
    try:
        ...
        return {
            ...
            "note": "Accepted formats: PNG, JPG, JPEG, WEBP. PDF is not allowed."
        }
...
def render_test(
    ...
    position: PositionEnum = Form(PositionEnum.right_chest),
):
    position_value = position.value
    garment_path = get_garment_template_for_position(position_value)
    logo_path = ASSETS_DIR / "logo.png"

    if not logo_path.exists():
        raise HTTPException(status_code=404, detail="logo.png not found in assets/")
...
async def render_mockup(
    ...
    validate_position(position_value)
